...
sum(Overall_FI_Score == '6', na.rm=TRUE) #14
sum(Overall_FI_Score == '5', na.rm=TRUE) #8
sum(Overall_FI_Score == '4', na.rm=TRUE) #13
sum(Overall_FI_Score == '3', na.rm=TRUE) #13
sum(Overall_FI_Score == '2', na.rm=TRUE) #34
sum(Overall_FI_Score == '1', na.rm=TRUE) #44
sum(Overall_FI_Score == '0', na.rm=TRUE) #495


sum(Cornell_FI == '6', na.rm=TRUE) # 0
sum(Cornell_FI == '5', na.rm=TRUE) # 0
sum(Cornell_FI == '4', na.rm=TRUE) # 0
sum(Cornell_FI == '3', na.rm=TRUE) # 0
sum(Cornell_FI == '2', na.rm=TRUE) # 16
sum(Cornell_FI == '1', na.rm=TRUE) # 75
sum(Cornell_FI == '0', na.rm=TRUE) #568

sum(Home_FI == '6', na.rm=TRUE) # 0
sum(Home_FI == '5', na.rm=TRUE) # 0
sum(Home_FI == '4', na.rm=TRUE) # 0
sum(Home_FI == '3', na.rm=TRUE) # 0
sum(Home_FI == '2', na.rm=TRUE) # 2
sum(Home_FI == '1', na.rm=TRUE) #14
sum(Home_FI == '0', na.rm=TRUE) #575
...
AK_popped_Cornell.Full <- glm(BART_balloons_popped  ~ Radimer_cornell_food_insecure*AK_insight_plan_control, family="poisson", data=FI)
summary(AK_popped_Cornell.Full)
#no interaction
...
#Notes - Go back and check for differences for specifically for minority students
